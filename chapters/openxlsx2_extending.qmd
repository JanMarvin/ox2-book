...
## `gtxlsx` - Exporting `gt`, `lt` and html to workbooks

Using `gtxlsx` [@gtxlsx] you can extend `openxlsx2` to write `gt` objects [@gt], `lt` objects [@lt] and html to spreadsheets. Various styling options are supported.

```{r}
library(gt)
library(gtxlsx)

tbl <- gt::gt(head(gtcars[, c("mfr", "model", "year", "msrp")])) |>
  gt::tab_header(title = md("The Cars of **gtcars**")) |>
  gt::fmt_currency(columns = msrp, decimals = 0) |>
  gt::tab_style(
    style = list(cell_fill(color = "lightblue"), cell_text(weight = "bold")),
    locations = gt::cells_body(columns = msrp, rows = msrp > 100000)
  )

wb <- wb_workbook()$add_worksheet(grid_lines = FALSE)
wb <- gtxlsx::wb_add_gt(wb, tbl, dims = "B2")
...
wb <- gtxlsx::wb_add_html(wb, html, dims = "A1")
...
## `oxview` - view `wbWorkbook`s in html

[@oxview] bundles the javascript library `office-open-xml-viewer` (https://github.com/yukiyokotani/office-open-xml-viewer) in an R package that allows viewing and interacting with spreadsheets via the RStudio viewer pane or any html browser. The library allows not only viewing `openxlsx2` files, but all Office Open XML files. So `pptx` and `docx` are supported as well. At the moment (Sep 2026) the library is actively developed and there are still various fixes implemented.

```{r}
if (interactive()) oxview::ox_view(wb)
```
