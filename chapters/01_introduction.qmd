...
[^01_introduction-2]: And there are many other packages on CRAN for working with `xls`/`xlsx` spreadsheet files. Without a guarantee for completeness: `SheetReader` [@sheetreader], `tablexlsx` [@tablexlsx], `xlsx2dfs` [@xlsx2dfs], `tablaxlsx` [@tablaxlsx], `xlr` [@xlr], `xlcutter` [@xlcutter], `knitxl` [@knitxl], `xlcharts` [@xlcharts], `joinXL` [@joinxl].
