...
We offer two distinct workflows for interacting with Excel files:
...
We strive to limit our dependencies to the bare minimum required to run the package. Currently, these are `R6`, `Rcpp`, and `stringi`. All three are dependency-free themselves (apart from depending on R) and should be available on any recent R setup that includes `devtools`. (In fact, many popular packages with extensive dependency trees—such as the `tidyverse`, `roxygen2`, `quarto`, or `shiny`—already include at least two of these).

In addition, the `zip` package is required to create xlsx output files if a system tool like `zip` or `bsdtar` (or `tar` on Windows) is unavailable. To build `openxlsx2` from source, a C++11 compatible compiler must be available. Other dependencies are optional and limited to specific features, or to our test suite, involving `ggplot2`, `mschart`, `openssl`, or `rvg`.

```{r}
db <- tools::CRAN_package_db()
deps <- tools::package_dependencies(package="openxlsx2", recursive=TRUE, db = db)$openxlsx2
# ignore packages shipped by R
setdiff(deps, c("grDevices", "utils", "methods", "tools", "stats"))
```
