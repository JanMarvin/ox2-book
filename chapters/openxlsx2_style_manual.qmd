...
It's also possible to change the entire theme of the workbook. This impacts the default colors and fonts in a workbook. `openxlsx2` ships a few styles from older and current spreadsheet software releases. If one wants to use the `LibreOffice` style sheet, this can be changed as follows:
...
An example how `openxlsx2` got the themes and colors from spreadsheets can be found in the `inst` folder, this should make it possible to extract or create custom themes, for example, aligned with corporate identity.

``` r
system.file("extdata/themes.R", package = "openxlsx2")
```
