...
## Adding plots via `{rvg}`, `{devEMF}` or `{easeling}`

If you want vector graphics that can be modified in spreadsheet software, all three packages provide a custom device that can be used with the majority of R plots. The packages differ in the way they construct the output, either as XML or EMF, their dependencies and slightly in the number of supported plotting features. The youngest of the three `{easeling}` [@easeling]  does not require additional dependencies and supports several chart types.
...
wb$add_worksheet("add_easeling")$add_drawing(xml = f, dims = "A1")
```

```{r echo=FALSE, warning=FALSE}
#nolint start
#| fig-cap: "An easeling chart is a vector graphic that can be modified in spreadsheet software"
knitr::include_graphics("../img/chrt_easeling.png")
#nolint end
```

For `{rvg}` [@rvg] , the `dml_xlsx()` device comes in handy. You can pass the output via `wb_add_drawing()`.
