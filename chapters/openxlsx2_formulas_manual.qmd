...
What happened? Even though we see cells `A1` and `B1` show a value of `2` and `1` our formula in `C1` was not updated. It still shows a value of `2`. This is because `openxlsx2` does not evaluate formulas and workbooks on a more general scale. In the openxml style the cell looks something like this:
...
There is a helper package to support modern spreadsheet formulas called `spaghetti` see @sec-spaghetti.
...
However, if we were to pass this as the text for the formula to be written it would cause an error because the syntax that openxml requires for selecting the current row is different.
...
Now we can see that openxml replaces `[@[sales_price]]` with `daily_sales[[#This Row],[sales_price]]` We must then use this syntax when writing formulas for `dataTable`
...
A helper for debugging spreadsheet formulas is `tidyxl::xlex()`.
...
wb <- wb_workbook()$
  ...
  add_formula(
    dims = "N2",
    x = 'VLOOKUP("Valiant", $A$1:$L$33, MATCH("wt", $1:$1, 0), FALSE)'
  )

wb$to_df(dims = "N2", show_formula = TRUE, col_names = FALSE) |> 
  unlist() |> 
  tidyxl::xlex()
```

## Example: Whatever I try, my formula wont work, what to do?

Sometimes a formula doesn’t behave as expected. You may notice unexpected "\@" symbols appearing when you type it, or the formula simply looks wrong. What can you do? The easiest approach is to enter the formula directly into a spreadsheet, verify that it works, save the file, and then load it using `wb_to_df(show_formula = TRUE, ...)` as shown earlier.

For a deeper look, you can load the workbook with `wb <- wb_load(...)`. Once loaded, you can inspect the internal `cc` data frame, which contains all cell information. If your workbook has only one worksheet, the code below allows you to examine exactly how the formula is represented in the file and how you might reconstruct it if needed.

```{r}
# from the wb_to_df() manual page:
example_file <- system.file("extdata", "openxlsx2_example.xlsx", package = "openxlsx2")
wb <- wb_load(example_file)

cc <- wb$worksheets[[1]]$sheet_data$cc

# inspect a cell with a formula
cc[cc$r == "I11", ]
```
