...
In addition to importing directly from xlsx, xlsm or xlsb files, `openxlsx2` provides the `wbWorkbook` class used for importing and modifying entire openxml files in `R`. This `workbook` class is the heart of `openxlsx2` and probably the reason why you are reading this manual in the first place.
...
```{r}
dims <- "A7:BG605"
df <- wb_to_df(wb, dims = dims, na = c("-", "**", "#N/A"),
...
```{r}
## adaptation of https://github.com/JanMarvin/openxlsx2/discussions/710
...
# read body
dims <- "A8:BG605"
df_body <- wb_to_df(wb, dims = dims, na = c("-", "**", "#N/A"),
                    fill_merged_cells = TRUE, col_names = FALSE)

# read header
dims <- "A5:BG7"
df_head <- wb_to_df(wb, dims = dims, na = c("-", "**", "#N/A"),
                    fill_merged_cells = TRUE, col_names = FALSE)
...
## Another real world data

Since release 1.22 it is possible to use `+` and `-` as wildcards in `dims`. This allows reading the following file is a single `wb_to_df()` call, even though the data is rather "untidy" spread on the sheet. Months are spread horizontally and there is a gap between the row names, the column names, and the data. [^openxlsx2_reading-2].

[^openxlsx2_reading-2]: A backup of the file can be found here <https://janmarvin.github.io/openxlsx-data/cpb-world-trade-monitor-september-2025.xlsx>.

```{r}
fl <- "https://www.cpb.nl/system/files/cpbmedia/cpb-world-trade-monitor-september-2025.xlsx"

wb <- wb_load(fl)

# there is a space between the dim names and the data
df <- wb$to_df(dims = "B4,F4:+4,B8,F8:+8", skip_empty_cols = TRUE, row_names = TRUE)

# the data is spread horizontally
df <- as.data.frame(t(df))
df$date <- paste0(rownames(df), "d01")
df$date <- as.Date(df$date, format = "%Ym%md%d")

plot(
  df$date, df$`World trade`, 
  main = "CPB.nl: World Trade seasonally adjusted",
  xlab = "Months",
  ylab = "Volumes (USD bln, 2021 = 100)",
  type = "l",
  panel.first = grid()
)
```
