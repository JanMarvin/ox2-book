...
There is a custom style helper `wb_add_style()` available at <https://github.com/JanMarvin/openxlsx2/discussions/1418>, not to be confused with the `openxlsx2` function with an identical name, meaningful function names are hard to find). With this, it should be possible to keep the `openxlsx` style objects and convert them to `openxlsx2` style functions when saving. This function uses some internal `openxlsx2` functions and should only be used for a transition phase, when switching from `openxlsx` to `openxlsx2`.
