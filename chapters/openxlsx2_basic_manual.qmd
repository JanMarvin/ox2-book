...
If you do not have a workbook yet, it is possible to create one. In the next line we will use three wrapper functions `wb_workbook()`, `wb_add_worksheet()`, and `wb_add_data()`. The wrapper functions are piped together using `R`'s native pipe operator `|>`, but similarly you can use the classic `magrittr` pipe operator `%>%`. [^openxlsx2_basic_manual-1] We assume that you have a dataset `your_data`, either a vector, a matrix or a data frame and want to write this in a worksheet:

[^openxlsx2_basic_manual-1]: Basically a pipe operator allows to write code from left to right. Without pipes the code would look like this:
...
In addition to importing directly from xlsx or xlsm files, `openxlsx2` provides the `wbWorkbook` class used for importing and modifying the entire openxml files in `R`. This `workbook` class is the heart of `openxlsx2` and probably the reason why you are reading this manual in the first place.
...
Prior to release 1.22, `wb_save()` relied on the `zip` package [@zip] to create the output `xlsx` file. Later releases prioritize system tools for zipping the output. 

The default is `utils::zip()`, which requires a zip tool to be available via `Sys.which("zip")` or `Sys.getenv("R_ZIPCMD")`. An alternative is `bsdtar`, which typically requires `Sys.which("bsdtar")` on Mac and Linux. On recent Windows versions, the binary is called `tar` and should be available via `Sys.which("tar")`.

On Windows, it is also possible to use the open-source utility `7zip` if it is configured in `R_ZIPCMD`. If none of these system-level tools work for your environment, falling back to the `zip` package is still supported. These configuration options are documented in the `wb_save()` manual pages. This entire approach was refined in subsequent releases and should work as intended starting from release 1.25.


## `dims`/ `wb_dims()`

In `openxlsx2` functions that interact with worksheet cells are using `dims` as argument and require the users to provide these. `dims` are cells or cell ranges in A1 notation. [^openxlsx2_basic_manual-2] [^openxlsx2_basic_manual-3] Since A1 notation is rather simple in the first few columns it might get confusing after the 26. Therefore we provide a wrapper to construct it:

[^openxlsx2_basic_manual-2]: The single argument `dims` hereby replaces the mix of `col`/`row`, `cols`/`rows` and `xy` used in `openxlsx`.

[^openxlsx2_basic_manual-3]: No rule without an exception: some functions still accept `cols` or `rows`, but only if `dims` would not make sense.
